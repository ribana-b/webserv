...
HttpServer::HttpServer(const Config& config, const Logger& logger) 
    : m_Config(config), m_Logger(logger), m_DocumentRoot("/var/www/html"), m_DefaultIndex("index.html") {
...
HttpServer::HttpServer(const HttpServer& that) 
    : m_Config(that.m_Config), m_Logger(that.m_Logger), 
      m_DocumentRoot(that.m_DocumentRoot), m_DefaultIndex(that.m_DefaultIndex) {}
...
HttpResponse HttpServer::processRequest(const HttpRequest& request, int serverPort) {
    m_Logger.info() << "Processing " << request.getMethod() << " " << request.getPath() 
                   << " HTTP/" << request.getVersion() << " on port " << serverPort;
...
void HttpServer::setDefaultIndex(const std::string& index) {
    m_DefaultIndex = index;
}

const std::string& HttpServer::getDocumentRoot() const {
    return m_DocumentRoot;
}

const std::string& HttpServer::getDefaultIndex() const {
...
HttpResponse HttpServer::handlePOST(const HttpRequest& request, const Config::Server& server) {
    ...
    m_Logger.info() << "POST request to " << requestPath << " (body: " << request.getBody().length() << " bytes)";
    ...
    if (requestPath == "/upload") {
        ...
        if (success) {
            ...
            responseBody << "<h1>Upload Successful!</h1><p>File saved as: " << filename 
                        ...
                        << "<p>Type: " << (isLargeUpload ? "Large file (streamed to disk)" : "Small file (in memory)") << "</p>";
...
HttpResponse HttpServer::serveStaticFile(const std::string& filePath, const Config::Server& server) {
    ...
    m_Logger.info() << "Served file: " << filePath << " (" << response.getContentLength() << " bytes)";
    return response;
}

HttpResponse HttpServer::generateDirectoryListing(const std::string& dirPath, const std::string& requestPath, const Config::Server& server) {
    (void)server;
    ...
    for (std::vector<std::string>::const_iterator it = directories.begin(); 
        ...
        html << "<a href=\"" << linkPath << "/\" class=\"directory\">" 
             << *it << "/" << sizeInfo << "</a>\n";
    }
...
const Config::Location* HttpServer::findMatchingLocation(const Config::Server& server, const std::string& path) const {
...
bool HttpServer::isMethodAllowed(const std::string& method, const Config::Location& location) const {
...
bool HttpServer::isCGIFile(const std::string& filePath) const {
    ...
    return (extension == ".php" || extension == ".py" || 
            extension == ".cgi" || extension == ".pl");
}

std::string HttpServer::resolvePath(const std::string& requestPath, const Config::Location& location) const {
...
HttpResponse HttpServer::testGenerateDirectoryListing(const std::string& dirPath, const std::string& requestPath, const Config::Server& server) {
    return generateDirectoryListing(dirPath, requestPath, server);
}

HttpResponse HttpServer::testServeStaticFile(const std::string& filePath, const Config::Server& server) {
    return serveStaticFile(filePath, server);
}
...
bool HttpServer::testIsPathSafe(const std::string& path) const {
    return isPathSafe(path);
}

std::string HttpServer::testResolvePath(const std::string& requestPath, const Config::Location& location) const {
...
HttpResponse HttpServer::handleCGI(const HttpRequest& request, const Config::Server& server, const std::string& filePath) {
...
HttpResponse HttpServer::createErrorResponse(int statusCode, const Config::Server& server) {
    ...
    if (it != server.errorPages.end()) {
        ...
        if (stat(errorPagePath.c_str(), &errorStat) == 0 && 
            S_ISREG(errorStat.st_mode) && 
        ...
        m_Logger.warn() << "Custom error page not accessible: " << errorPagePath << " for status " << statusCode;
    }
    
    // Fallback to default error responses
    switch (statusCode) {
        case 400: return HttpResponse::createBadRequest();
        case 403: {
            HttpResponse response = HttpResponse::createBadRequest();
            response.setStatus(403, "Forbidden");
            return response;
        }
        case 404: return HttpResponse::createNotFound();
        case 405: return HttpResponse::createMethodNotAllowed();
        case 413: {
            HttpResponse response = HttpResponse::createInternalError();
            response.setStatus(413, "Payload Too Large");
            return response;
        }
        case 500: return HttpResponse::createInternalError();
        default:  return HttpResponse::createInternalError();
    }
}
